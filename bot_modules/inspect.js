...
const { findGame, findMember, findRole, findUser } = require('../bot_utils/discord-utils')
const { formatTimestamp, stringify } = require('../bot_utils/format')
const { reply } = require('../bot_utils/messaging')

function install(registerMessageHandler) {
    console.log('[TRACE] inspect.install(registerMessageHandler)')
    registerMessageHandler('i', handleInspectMessage)
    registerMessageHandler('inspect', handleInspectMessage)
}
...
module.exports = { install, getGameProps }
